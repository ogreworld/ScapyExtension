...
class ObjectPipe:
    def __init__(self):
        self.is_wondows = sys.platform.startswith("win32")
        if self.is_wondows:
            import my_socketpair
            self.rd, self.wr = my_socketpair.SocketPair()
        else:
            self.rd, self.wr = os.pipe()
        self.queue = deque()
    def fileno(self):
        if self.rd is not None and type(self.rd) is not int:
...
class Automaton:
        ...
        while True:
            try:
                ...
                while 1:
                    ...
                    if sys.platform.startswith("win32"):
                        r = self._windows_select(fds, remain)
                    else:
    ...
    def _windows_select(self, rlist, timeout):
        select_r = []               
        ...
        while 1:
            ...
            if len(self.recv_conditions[self.state.state]) > 0:
                try:
                    pkt = self.listen_sock.recv(MTU)
                    select_r.append(self.listen_sock)
                    self.windows_pkt = pkt
                    return select_r
                except PcapTimeoutElapsed:
                    pass
            
            r,_,_ = select(rlist, [], [], 0.1)
            if r:
                return r
