...
class UTCTimeField(IntField):
    epoch = (2000, 1, 1, 12, 0, 0, 5, 1, 0) # required Epoch
    epoch2 = (1970, 1, 1, 12, 0, 0, 3, 1, 0 )
    def i2repr(self, pkt, x):
        x = self.i2h(pkt, x)
        from time import gmtime, strftime, mktime
        delta = mktime(self.epoch) - mktime(self.epoch2)
        x = x + delta
...
class DHCP6_Reply(DHCP6):
    ...
    def answers(self, other):
        return ((isinstance(other, DHCP6_InfoRequest) or
                isinstance(other, DHCP6) ) and
                self.trid == other.trid)
   ...
   If you have a need for others, just ask ... or provide a patch."""
    ...
    def parse_options(self, dns="2001:500::1035", domain="localdomain, local",
        ...
        if duid is not None:
            ...
            epoch = (2000, 1, 1, 12, 0, 0, 5, 1, 0) # required Epoch
            epoch2 = (1970, 1, 1, 12, 0, 0, 3, 1, 0 )            
            delta = mktime(epoch) - mktime(epoch2)
