...
class IP(Packet, IPTools):
    ...
    fields_desc = [ BitField("version" , 4 , 4),
                    ...
                    # BitField("flagrsvd",0,1),
                    # BitEnumField("DF",0,1,{0:"Don't Fragment not set(0)",1:"Don't Fragment set(1)"}),
                    # BitEnumField("MF",0,1,{0:"More Fragment not set(0)",1:"More Fragment set(1)"}),
                    BitField("frag", 0, 13),
                    ByteField("ttl", 64),
    ...
    def send(self, s, slp=0):
        for p in self:
            ...
            if slp:
                time.sleep(slp)
    def route(self, iface_hint=None):
        dst = self.dst
        if isinstance(dst,Gen):
            dst = iter(dst).next()
        return conf.route.route(dst, iface_hint=iface_hint)
    def hashret(self):
    ...
    def fragment(self, fragsize=1480):
        ...
        if self.payload.haslayer(IP):  # PPTP, L2TP need to fragment the payload IP layer.          
            return self.payload.fragment(fragsize=fragsize)
...
class IGMP(Packet):
    ...
    fields_desc = [ ByteEnumField("type", 0x11 ,igmptypes),
                    ByteField("max_resp_time",0x64),
                    XShortField("chksum", None),
                    IPField("group_addr","0.0.0.0"),
                    StrField("other_fields",""),]
    def post_build(self, p, pay):
        p += pay
        if self.chksum is None:
            ck = checksum(p)
