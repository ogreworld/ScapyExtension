...
class Route:
    ...
    def resync(self):
        ...
        self.sync_default_iface()
        
    def __repr__(self):
        rt = "Network         Netmask         Gateway         Iface           Output IP       Metric\n"
        for net,msk,gw,iface,addr,metric in self.routes:
            rt += "%-15s %-15s %-15s %-15s %-15s %-15s\n" % (ltoa(net),
                                              ltoa(msk),
                                              gw,
                                              iface,
                                              addr,
    ...
    def make_route(self, host=None, net=None, gw=None, dev=None, metric=20):
        if host is not None:
            thenet,msk = host,32
    ...
    def ifchange(self, iff, addr):
        ...
        for i in range(len(self.routes)):
            net,msk,gw,iface,addr, metric = self.routes[i]
            if iface != iff:
                continue
            if gw == '0.0.0.0':
    ...
    def sync_default_iface(self):
        _betteriface = self.route("0.0.0.0",verbose=0)[0]
