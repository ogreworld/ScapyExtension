...
if struct.pack("H",1) == "\x00\x01": # big endian
    ...
    def checksum(pkt):
        ...
        # for i in range(0, len(pkt), 2  ):            
            # tmp = struct.unpack('<H', str(pkt)[i:i+2])
            # s += tmp[0]
            # print tmp
        s = sum(array.array("H", pkt))
        ...
        # print (((s>>8)&0xff)|s<<8) & 0xffff
        return (((s>>8)&0xff)|s<<8) & 0xffff


@conf.commands.register
def ip_checksum(pkt):
    ''' return 0 if checksum of IP is corrent '''
    if pkt.haslayer(scapy.layers.inet.IP) :        
        return checksum( str(pkt[scapy.layers.inet.IP])[:20] )   
        
    raise ValueError("Layer IP not found" )    
    
    
@conf.commands.register
def tcp_checksum(pkt):
    ''' return 0 if checksum of TCP is corrent '''
    if not pkt.haslayer(scapy.layers.inet.TCP):
        raise ValueError("Layer TCP not found" )    
    
    p = pkt[scapy.layers.inet.TCP]
    
    if isinstance(p.underlayer, scapy.layers.inet.IP):
        if p.underlayer.len is not None:
            ln = p.underlayer.len-20
        else:
            ln = len(p)
        psdhdr = struct.pack("!4s4sHH",
                             inet_aton(p.underlayer.src),
                             inet_aton(p.underlayer.dst),
                             p.underlayer.proto,
                             ln)
        ck=checksum(psdhdr+str(p))        
    elif conf.ipv6_enabled and isinstance(p.underlayer, scapy.layers.inet6.IPv6) or isinstance(p.underlayer, scapy.layers.inet6._IPv6ExtHdr):
        ck = scapy.layers.inet6.in6_chksum(socket.IPPROTO_TCP, p.underlayer, p)
    else:
        raise ValueError("Underlayer is not IP/IPv6" ) 
    return ck
    
@conf.commands.register
def udp_checksum(pkt):
    ''' return 0 if checksum of UDP is corrent '''
    if not pkt.haslayer(scapy.layers.inet.UDP):
        raise ValueError("Layer UDP not found" )    
    
    p = pkt[scapy.layers.inet.UDP]
    ...
    if isinstance(p.underlayer, scapy.layers.inet.IP):
        ...
        ck = scapy.layers.inet6.in6_chksum(socket.IPPROTO_UDP, p.underlayer, p)
    else:
        raise ValueError("Underlayer is not IP/IPv6" ) 
    return ck
