...
class NetField(Field): # Added by chenzongze 2014.1.1
    ...
    def getfield(self, pkt, s):
        ...
        len =  (self.length_from(pkt) +7)/8
        return s[len:], self.m2i(pkt,s[:len])
    ...
    def i2h(self, pkt, x):
        mask_width = self.length_from(pkt)
        shift_width = 32-mask_width
        return ltoa(atol(x) & (0xffffffff>>shift_width<< shift_width))
...
class StrEnumField(StrField):
    def __init__(self, name, default, fmt="H", enum=None, remain=0):
