...
def win_find_exe(filename, installsubdir=None, env="ProgramFiles"):
    ...
    for fn in [filename+".exe", filename]:
        try:
...
class NetworkInterface(object):
    ...
    def __repr__(self):
        return "<%s: %s %s %s pcap_name=%s win_name=%s\n>" % (self.__class__.__name__,
                     self.name, self.ip_list, self.mac, self.pcap_name, self.win_name)
...
def sniff(count=0, store=1, offline=None, prn = None, lfilter=None, L2socket=None, 
                                        timeout=None, stop_filter=None, stopper=None, *arg, **karg):
    """Sniff packets
