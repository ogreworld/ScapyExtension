...
def __gen_send(s, x, inter=0, loop=0, count=None, verbose=None, realtime=None, timeout=None, stop_filter=None, *args, **kargs):
    if type(x) is str:
        x = Raw(load=x)
...
def send(x, inter=0, loop=0, count=None, verbose=None, realtime=None, timeout=None, stop_filter=None, *args, **kargs):
    """Send packets at layer 3
send(packets, [inter=0], [loop=0], [verbose=conf.verb]) -> None"""
    __gen_send(conf.L3socket(*args, **kargs), x, inter=inter, loop=loop, count=count,verbose=verbose, realtime=realtime, timeout=timeout, stop_filter=stop_filter)

@conf.commands.register
def sendp(x, inter=0, loop=0, iface=None, iface_hint=None, count=None, verbose=None, realtime=None, timeout=None, stop_filter=None, *args, **kargs):
    """Send packets at layer 2
sendp(packets, [inter=0], [loop=0], [verbose=conf.verb]) -> None"""
    if iface is None and iface_hint is not None:
        iface = conf.route.route(iface_hint)[0]
    __gen_send(conf.L2socket(iface=iface, *args, **kargs), x, inter=inter, loop=loop, count=count, verbose=verbose, realtime=realtime, timeout=timeout, stop_filter=stop_filter)
...
stop_filter: python function applied to each packet to determine
    ...
    conf.stop_sniff = False
       
    while 1:
        try:
            if stopper and callable(stopper) and stopper():
                break
            ...
            conf.is_sniffing    =   True
            #sel = select([s],[],[],remain )
            # select function will not return until remain timeout, while stopper will not work
            # if no packets captured, contributed by @liuquanquan 2013.9.27            
            sel = select([s],[],[],remain if remain < 1 else 1)
...
@conf.commands.register
def is_sniffing():
    """To judge whether scapy is sniffing packets """
    
    status = conf.is_sniffing
    # if status:
        # time.sleep(1)
...
@conf.commands.register
def stop_sniff():
    """To force stop sniffing packets """
    conf.stop_sniff =  True
    max_retry = retry = 15
    while retry:
        time.sleep(1)
        if not is_sniffing():
    ...
    raise RuntimeError('Stop siniffing fail in %s seconds.' % max_retry)    